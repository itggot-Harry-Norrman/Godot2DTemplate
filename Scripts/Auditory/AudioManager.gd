...
func create_2d_audio_at_location(location: Vector2, type: SoundEffect.SOUND_EFFECT_TYPE) -> void:
	if sound_effect_dict.has(type):
		...
		if sound_effect.has_open_limit():
			...
			new_2D_audio.pitch_scale += Globals.rng.randf_range(-sound_effect.pitch_randomness, sound_effect.pitch_randomness )
...
func create_2d_audio_loop(location: Vector2, type: SoundEffect.SOUND_EFFECT_TYPE) -> AudioStreamPlayer2D:
	if sound_effect_dict.has(type):
		var sound_effect: SoundEffect = sound_effect_dict[type]
		#sound_effect.change_audio_count(1)
		var new_2D_audio: AudioStreamPlayer2D = AudioStreamPlayer2D.new()
...
func stop_2d_loop(loop : AudioStreamPlayer2D):
	loop.stop()
	loop.call_deferred("queue_free")
...
func create_audio(type: SoundEffect.SOUND_EFFECT_TYPE) -> void:
	if sound_effect_dict.has(type):
		...
		if sound_effect.has_open_limit():
			...
			new_audio.pitch_scale += Globals.rng.randf_range(-sound_effect.pitch_randomness, sound_effect.pitch_randomness )
